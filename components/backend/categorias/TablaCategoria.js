    ...
    >
      <div className="ml-6 mt-3 flex flex-col flex-grow h-auto mr-4 bg-white flex-shrink shadow-lg rounded-xl">
        <div className="h-16 flex justify-left items-center">
          <h1 className="ml-4 text-2xl font-bold text-red-500">
            Lista de categorias
          </h1>
        </div>
              ...
              <ul className="mt-5" {...droppableProvided.droppableProps} ref={droppableProvided.innerRef} >
                {categorias.map((categoria, index) => (
                  <Draggable key={categoria.id} draggableId={categoria.id} index={index} >
                    {(draggableProvided) => (
                      <li className="" {...draggableProvided.draggableProps} ref={draggableProvided.innerRef} {...draggableProvided.dragHandleProps} >
                        <Categoria categoria={categoria} index={index} />
                      </li>
                    )}
                  </Draggable>
                ))}
